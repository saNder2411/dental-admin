...
import { watchSkills } from '../_bus/_Skills/SkillsSagaWatchers';
import { watchAuth } from '../_bus/User/UserSagaWatchers';

export function* rootSaga() {
  try {
    yield all([call(watchAuth), call(watchAppointments), call(watchCustomers), call(watchStaff), call(watchServices), call(watchSkills)]);
  } catch (error) {
