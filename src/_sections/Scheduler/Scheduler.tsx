  ...
  const onDataChange = useCallback(
    ({ updated, created }: SchedulerDataChangeEvent) => {
      ...
      if (updatedDataItem.MetroRRule && created[0]) {
        ...
        dispatch(updateAppointmentRecurringDataItemInitAsyncAC(updatedDataItem, processCreateDataItem, () => setIsAgendaDataItemLoading(false)));
        return;
      }

      setIsAgendaDataItemLoading(true);
      const processUpdateDataItem = { ...updatedDataItem, LookupHR01teamId: updatedDataItem.TeamID, type: TypesProcessDataItem.Grid };

      dispatch(updateAppointmentDataItemInitAsyncAC(processUpdateDataItem, () => setIsAgendaDataItemLoading(false)));
    },
    [appointmentsAllIDs, dispatch, setIsAgendaDataItemLoading]
  );
